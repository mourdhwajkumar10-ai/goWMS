...
// src/types/index.ts
var AI_TYPES = ["claude", "cursor", "windsurf", "antigravity", "copilot", "roocode", "kiro", "codex", "qoder", "gemini", "trae", "opencode", "continue", "codebuddy", "droid", "kilocode", "warp", "augment", "codewhale", "universal", "all"];
var AI_FOLDERS = {
  claude: [".claude"],
...
async function renderSkillFile(config, isGlobal = false) {
  ...
  const rootedScriptPath = `${config.folderStructure.root}/${config.scriptPath}`.replace(/\/{2,}/g, "/");
...
` + quickReferenceContent : "";
  content = content.replace(/\{\{TITLE\}\}/g, config.title).replace(/\{\{DESCRIPTION\}\}/g, config.description).replace(/\{\{SCRIPT_PATH\}\}/g, commandScriptPath).replace(/\{\{SKILL_OR_WORKFLOW\}\}/g, config.skillOrWorkflow).replace(/\{\{QUICK_REFERENCE\}\}/g, quickRefWithNewline);
...
async function generatePlatformFiles(targetDir, aiType, isGlobal = false, force = false) {
  ...
  const dataDir = config.folderStructure.dataPath ? join2(effectiveDir, config.folderStructure.root, config.folderStructure.dataPath) : skillDir;
  await mkdir2(dataDir, { recursive: true });
  await copyDataAndScripts(dataDir);
  const skillsParentDir = join2(effectiveDir, config.folderStructure.root, config.folderStructure.dataPath ? dirname(config.folderStructure.dataPath) : dirname(config.folderStructure.skillPath));
...
async function generateAllPlatformFiles(targetDir, isGlobal = false, force = false) {
  ...
  const generatedSkillFiles = new Set;
  for (const aiType of Object.keys(AI_TO_PLATFORM)) {
    try {
      const config = await loadPlatformConfig(aiType);
      const skillFile = join2(config.folderStructure.root, config.folderStructure.skillPath, config.folderStructure.filename);
      if (generatedSkillFiles.has(skillFile))
        continue;
...
function detectAIType(cwd = process.cwd()) {
  ...
  if (existsSync2(join3(cwd, ".agents"))) {
    detected.push("antigravity");
    detected.push("codex");
    detected.push("universal");
  } else if (existsSync2(join3(cwd, ".agent"))) {
    detected.push("antigravity");
  }
  ...
  if (existsSync2(join3(cwd, ".codex")) && !detected.includes("codex")) {
    detected.push("codex");
  }
  ...
  if (existsSync2(join3(cwd, ".codewhale"))) {
    detected.push("codewhale");
  }
  let suggested = null;
  if (detected.length === 1) {
    suggested = detected[0];
  } else if ((detected.length === 2 || detected.length === 3) && detected.includes("antigravity") && detected.includes("codex")) {
    suggested = "codex";
...
async function getPackageVersion() {
  const packageCandidates = [
    join5(__dirname4, "..", "package.json"),
    join5(__dirname4, "..", "..", "package.json")
  ];
  const packagePath = packageCandidates.find((path) => existsSync4(path)) ?? packageCandidates[0];
...
async function updateCommand(options) {
  ...
  try {
    ...
    if (currentVersion !== latestVersion) {
      ...
      logger.info(`Now rerun ${source_default.cyan(`uipro init${aiFlag} --force${globalFlag}`)} to refresh your skill files.`);
      return;
    }
    console.log();
...
async function removeSkillDir(baseDir, aiType) {
  ...
  try {
    ...
    if (folderStructure.dataPath) {
      skillFiles.add(join6(folderStructure.root, folderStructure.skillPath, folderStructure.filename));
      parents.add(join6(folderStructure.root, dirname4(folderStructure.dataPath)));
    } else {
  ...
  for (const skillFile of skillFiles) {
    const filePath = join6(baseDir, skillFile);
    try {
      await stat(filePath);
...
program2.command("update").description("Update UI/UX Pro Max to latest version").option("-a, --ai <type>", `AI assistant type (${AI_TYPES.join(", ")})`).option("-g, --global", "Update global installation in home directory (~/)").option("-t, --token <token>", "GitHub Personal Access Token for higher API rate limits").action(async (options) => {
  if (options.ai && !AI_TYPES.includes(options.ai)) {
    console.error(`Invalid AI type: ${options.ai}`);
    console.error(`Valid types: ${AI_TYPES.join(", ")}`);
    process.exit(1);
