...
func createTrip(db *pgxpool.Pool) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if body.CarrierID != nil && body.CarrierName == "" {
			_ = db.QueryRow(c.Context(), `SELECT name FROM suppliers WHERE id=$1`, *body.CarrierID).Scan(&body.CarrierName)
		}

		var id int
		var tripNo string
		err := db.QueryRow(c.Context(),
			`INSERT INTO delivery_trips (trip_no,driver_id,vehicle_no,driver_name,status,carrier_id,carrier_name)
			 VALUES ('DT-'||TO_CHAR(NOW(),'YYYY')||'-'||LPAD(nextval('delivery_trips_id_seq')::TEXT,5,'0'),$1,$2,$3,'scheduled',$4,$5)
			 RETURNING id, trip_no`,
			body.DriverID, body.VehicleNo, nullEmpty(body.DriverName), body.CarrierID, nullEmpty(body.CarrierName)).Scan(&id, &tripNo)
		if err != nil {
			// Fallback if carrier columns missing
			err = db.QueryRow(c.Context(),
...
func completeTrip(db *pgxpool.Pool) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		dns, _ := autoGenerateDNsForTrip(c.Context(), db, id)
		var tripNo string
		_ = db.QueryRow(c.Context(), `SELECT trip_no FROM delivery_trips WHERE id=$1`, id).Scan(&tripNo)
		notifications.EmitTripComplete(c.Context(), db, tripNo)
		return shared.OK(c, fiber.Map{"id": id, "status": "completed", "delivery_notes": dns})
	}
}
...
func generateDN(db *pgxpool.Pool) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		dnID, dnName, err := insertDNForTrip(c.Context(), db, tripID, body.Customer, body.AgainstSO)
		if err != nil {
			return shared.Err(c, fiber.StatusInternalServerError, err.Error())
		}
...
func completeTripGated(db *pgxpool.Pool) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		_ = db.QueryRow(c.Context(), `SELECT trip_no FROM delivery_trips WHERE id=$1`, id).Scan(&tripNo)
		notifications.EmitTripComplete(c.Context(), db, tripNo)
		return shared.OK(c, fiber.Map{"id": id, "status": "completed", "gated": true, "delivery_notes": dns})
	}
}

// autoGenerateDNsForTrip creates delivery notes for stops missing one.
// If the trip has loaded boxes but no stops, creates a single DN for the trip.
// Idempotent: stops that already have delivery_note_no are left alone.
func autoGenerateDNsForTrip(ctx context.Context, db *pgxpool.Pool, tripID int) ([]fiber.Map, error) {
	type stopRow struct {
		ID       int
		Customer *string
		DNNo     *string
	...
	rows, err := db.Query(ctx, `
		SELECT id, customer, delivery_note_no
		FROM delivery_stops WHERE trip_id=$1 ORDER BY stop_order, id`, tripID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var s stopRow
		if err := rows.Scan(&s.ID, &s.Customer, &s.DNNo); err != nil {
			return nil, err
		}
		stops = append(stops, s)
	}
	...
	created := []fiber.Map{}
	for _, s := range stops {
		if s.DNNo != nil && *s.DNNo != "" {
			created = append(created, fiber.Map{
				"delivery_note": *s.DNNo, "stop_id": s.ID, "existing": true,
			})
			continue
		}
		cust := ""
		...
		dnID, dnName, err := insertDNForTrip(ctx, db, tripID, cust, "")
		if err != nil {
			continue
		}
		_, _ = db.Exec(ctx, `
			...
			s.ID, dnName, tripID)
		created = append(created, fiber.Map{
			"delivery_note_id": dnID, "delivery_note": dnName, "stop_id": s.ID,
		})
	}

	if len(stops) == 0 {
		var loaded int
		_ = db.QueryRow(ctx, `SELECT COUNT(*) FROM box_load_logs WHERE trip_id=$1`, tripID).Scan(&loaded)
		if loaded > 0 {
			// Avoid duplicate trip-level DNs on re-complete edge cases
			var existing int
			_ = db.QueryRow(ctx, `
				SELECT COUNT(*) FROM delivery_notes WHERE trip_id=$1`, tripID).Scan(&existing)
			if existing == 0 {
				dnID, dnName, err := insertDNForTrip(ctx, db, tripID, "", "")
				if err == nil {
					created = append(created, fiber.Map{
						"delivery_note_id": dnID, "delivery_note": dnName, "stop_id": 0,
					})
				}
			}
		}
...
func insertDNForTrip(ctx context.Context, db *pgxpool.Pool, tripID int, customer, againstSO string) (int, string, error) {
	var dnID int
	var dnName string
	err := db.QueryRow(ctx, `
		...
		VALUES ('DN-'||TO_CHAR(NOW(),'YYYY')||'-'||LPAD(nextval('delivery_notes_id_seq')::TEXT,5,'0'),
			$1, 'draft', $2, $3)
		RETURNING id, name`,
		nullEmpty(customer), nullEmpty(againstSO), tripID).Scan(&dnID, &dnName)
	if err != nil {
		return 0, "", err
	}
	_, _ = db.Exec(ctx, `
		...
		WHERE bl.trip_id=$3`, dnID, againstSO, tripID)
	return dnID, dnName, nil
}
